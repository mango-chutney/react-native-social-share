...
RCT_EXPORT_METHOD(shareOnTwitter:(NSDictionary *)options
...
RCT_EXPORT_METHOD(shareBySMS:(NSDictionary *)options
                  callback: (RCTResponseSenderBlock)callback)
{
  MFMessageComposeViewController *composeCtl = [[MFMessageComposeViewController alloc] init];
  composeCtl.messageComposeDelegate = self;
  
  if ([options objectForKey:@"image"] && [options objectForKey:@"image"] != [NSNull null]) {
    [composeCtl addAttachmentData:[UIImage imageNamed: options[@"image"]] typeIdentifier:"kuTTypeJPEG" filename:"sponsor-me.jpg"];
  } else if ([options objectForKey:@"imagelink"] && [options objectForKey:@"imagelink"] != [NSNull null]) {
    NSString *imagelink = [RCTConvert NSString:options[@"imagelink"]];
    [composeCtl addAttachmentURL:[NSURL URLWithString:imagelink] withAlternateFilename:"sponsor-me.jpg"];
  }
  
  if ([options objectForKey:@"text"] && [options objectForKey:@"text"] != [NSNull null]) {
    NSString *text = [RCTConvert NSString:options[@"text"]];
    composeCtl.body = text;
  }
